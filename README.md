...
| 📚 课程 | C# 12 课 + Unity 12 课 + 算法 10 专题（共 **212 题**），顺序闯关（完成上一课解锁下一课），知识点卡片 + 选择题；每个知识点附**「深入了解」详解**（原理、常见坑、实战建议） |
| 🧪 质量保障 | `node tools/verify-qs.mjs` 自动校验全题库：答案索引合法性、选项查重，31 道"看代码猜输出"题用 JS 模拟器重算真实输出并与标定答案比对 |
